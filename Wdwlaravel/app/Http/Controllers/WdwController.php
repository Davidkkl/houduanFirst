...
class WdwController extends Controller
{
    ...
    protected function WdwUserRegister(Request $request){
        ...
        $count = Student::WdwUserCheckNumber($student_ID);
        if($count == 0){
            $dm = Student::WdwcreateUser($userdata);
            if($dm){
                var_dump('账号创建成功');
            }else{
                var_dump('账号创建失败');
    ...
    protected function WdwUserLogin(Request $request){
        ...
        $token = $user['token'];
        if($user['token']){
            $p = Student::token($user);
            //var_dump($p);
            if($p){
                //return response()->json(['message' => '登录成功！'],$user['token'], 200);
